...
def PiecewiseLinear(imgin):
    ...
    L = 256  # Giả sử L là 256, bạn cần đặt giá trị này tùy thuộc vào miền giá trị của ảnh
    ...
    for x in range(0, M):
        for y in range(0, N):
            r = imgin[x, y]
            if r < r1:
                if r1 != 0:  # Kiểm tra trước khi chia để tránh lỗi ZeroDivisionError
                    s = s1 / r1 * r
                else:
                    s = 0
            elif r < r2:
                if (r2 - r1) != 0:  # Kiểm tra trước khi chia để tránh lỗi ZeroDivisionError
                    s = (s2 - s1) / (r2 - r1) * (r - r1) + s1
                else:
                    s = 0
            else:
                if (L - 1 - r2) != 0:  # Kiểm tra trước khi chia để tránh lỗi ZeroDivisionError
...
if uploaded_file is not None:
    ...
    with col1:  st_image = st.image(imgin, caption="Hình ảnh tải lên", use_column_width=True)
    ...
    if technique == "Negative":
        ...
        with col1:  
            st_image.empty()
            st.image(frame, caption="Hình ảnh tải lên", use_column_width=True)
